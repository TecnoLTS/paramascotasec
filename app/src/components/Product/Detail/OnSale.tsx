...
const OnSale: React.FC<Props> = ({ data, productId }) => {
    ...
    const rawGallery = [
        ...(Array.isArray((productMain as any)?.images)
    ...
    const fallbackThumbs = Array.isArray((productMain as any)?.thumbImage) ? (productMain as any).thumbImage.filter(Boolean) : []
    const fallbackGallery = galleryImages.length > 0
        ? galleryImages
        : (fallbackThumbs.length > 0 ? fallbackThumbs : productMain.images)
                    ...
                    <div className="container flex justify-between gap-y-6 flex-wrap md:py-20 py-10">
                        ...
                        <div className="product-infor md:w-1/2 w-full lg:pl-[30px] md:pl-4">
                            ...
                            <div className="flex items-center gap-3 flex-wrap mt-5 pb-6 border-b border-line">
                                ...
                                <div className="product-origin-price font-normal text-secondary2"><del>${Number(productMain.originPrice ?? 0).toLocaleString('es-EC', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</del></div>
